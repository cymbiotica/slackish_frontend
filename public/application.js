...
var apiUrl = void 0;
var apiUrls = {
    production: 'https://slackish-backend.herokuapp.com/',
...
var channelUI = __webpack_require__(5);
var channelApi = __webpack_require__(6);
var getFormFields = __webpack_require__(2);
var store = __webpack_require__(1);

var onCreateChannel = function onCreateChannel(event) {
    event.preventDefault();
    var data = getFormFields(this);
    channelApi.createChannel(data).then(function (data) {
...
var onShowChannels = function onShowChannels(event) {
    // event.preventDefault()
    getChannels();
};

// generic event-less api request
var getChannels = function getChannels(data) {
    channelUI.hideChannels();
    channelApi.showChannels().then(function (data) {
        channelUI.createChannelList(data, onShowChannels);
    });
    // .catch(channelUI.onCreateChannelFailure)
};

var loadChannelMessages = function loadChannelMessages() {
    console.log('messages event worked');
};

var onGetChannel = function onGetChannel(event) {

    event.preventDefault();
    var data = getFormFields(event.target);
    var channelId = getChanId(data);

    channelApi.showChannel(channelId, data).then(channelUI.showChannel(channelId, data));
};
...
var onUpdateChannel = function onUpdateChannel(event) {
    event.preventDefault();
    var data = getFormFields(this);

    channelApi.updateChannel(getChanId(data), data).then(function () {
        channelUI.onUpdateChannelSuccess(data, getChannels);
    });
};

// helper
var getChanId = function getChanId(data) {
    var channelId = void 0;
    for (var i = 0; i < store.channels.length; i++) {
...
var onCreateChannelSuccess = function onCreateChannelSuccess(data, getChannels) {
...
var createChannelList = function createChannelList(data, showChannels) {
  var channelContainer = $('#channel-list');
  ...
  var _loop = function _loop(i) {
    ...
    deleteIcon.addEventListener('click', function (event) {
      api.deleteChannel(data.channels[i].id).then(function (event) {
        onDeleteChannelSuccess(event, showChannels);
      });
    });

    var channelLink = document.createElement('div');
...
var showChannel = function showChannel(channelId, data) {
...
var onDeleteChannelSuccess = function onDeleteChannelSuccess(event, showChannels) {
  // event.preventDefault()
  showChannels();
};

var onUpdateChannelSuccess = function onUpdateChannelSuccess(data, getChannels) {
  getChannels();
};

var hideChannels = function hideChannels() {
  var channelList = $('#channel-list');
  channelList.empty();
};
...
module.exports = {
  ...
  onDeleteChannelSuccess: onDeleteChannelSuccess,
  onUpdateChannelSuccess: onUpdateChannelSuccess,
...
var showChannel = function showChannel(channelId) {
  return $.ajax({
...
var createChannel = function createChannel(data) {
  return $.ajax({
    url: config.apiUrl + '/channels',
    method: 'POST',
    headers: {
...
var updateChannel = function updateChannel(channelId, data) {
  // console.log(data)
  return $.ajax({
    url: config.apiUrl + ('/channels/' + channelId),
    method: 'PATCH',
...
var getFormFields = __webpack_require__(2); // supplied module from GA
var api = __webpack_require__(13); // our user api
var ui = __webpack_require__(14); // user UI updates
...
var onSignup = function onSignup(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.signUp(data).then(ui.onSignUpSuccess).then(setTimeout(function () {
    return $('#sign-up-message').text('');
  }, 1000)).then(setTimeout(function () {
    return $('.modalDialog-te').hide();
  }, 1500)).catch(ui.onSignUpFailure);
};
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.signIn(data).then(ui.onSignInSuccess).then(setTimeout(function () {
...
var onChangePassword = function onChangePassword(event) {
  ...
  var data = getFormFields(this);
  api.changePassword(data).then(ui.onChangePasswordSuccess).then(setTimeout(function () {
    return $('#change-password-message').text('');
  }, 1000)).then(setTimeout(function () {
    return $('.modalDialog-te').hide();
  }, 1500)).catch(ui.onChangePasswordFailure);
};
...
var onSignOut = function onSignOut(event) {
  ...
  api.signOut().then(ui.onSignOutSuccess).then(setTimeout(function () {
    return $('#sign-out-message').text('');
  }, 1000)).then(setTimeout(function () {
    return $('.modalDialog-te').hide();
  }, 1500)).catch(ui.onSignOutFailure);
};
...
// 'use strict'
var store = __webpack_require__(1);
var channelEvents = __webpack_require__(4);
var channelUI = __webpack_require__(5);
